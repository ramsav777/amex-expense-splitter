...
def summary_tab():
    ...
    # Calculate grand total and transaction counts for debugging
    grand_total = sum(t['amount'] for t in st.session_state.transactions)
    total_count = len(st.session_state.transactions)
    needs_review_count = len([t for t in st.session_state.transactions if t['category'] == 'Needs Review'])
    
    # Show grand total at the top for verification
    st.markdown(f"""
    <div style="background-color: #1E1E1E; padding: 20px; border-radius: 10px; text-align: center; border: 3px solid #FFD700; margin-bottom: 20px;">
        <h2 style="margin: 0; color: #FFD700;">📋 Statement Grand Total</h2>
        <p style="font-size: 42px; font-weight: bold; margin: 10px 0; color: #FFFFFF;">£{grand_total:,.2f}</p>
        <p style="color: #AAAAAA; margin: 5px 0;">Total Transactions: {total_count} | Needs Review: {needs_review_count}</p>
        <p style="color: #AAAAAA; margin: 5px 0; font-size: 12px;">This should match your statement total (excluding any fees/interest)</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.caption("Shows assigned spending by category, not who's card was used")
    ...
    with col1:
        ...
        <div style="background-color: #262730; padding: 20px; border-radius: 10px; text-align: center; border: 2px solid #4A4A5E;">
            <h3 style="margin: 0; color: #FAFAFA;">👤 Person A</h3>
            <p style="font-size: 32px; font-weight: bold; margin: 10px 0; color: #FF6B9D;">£{personal_a:,.2f}</p>
            <p style="font-size: 12px; color: #AAAAAA; margin: 0;">Personal Spending</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col2:
        st.markdown(f"""
        <div style="background-color: #262730; padding: 20px; border-radius: 10px; text-align: center; border: 2px solid #4A4A5E;">
            <h3 style="margin: 0; color: #FAFAFA;">👥 Person B</h3>
            <p style="font-size: 32px; font-weight: bold; margin: 10px 0; color: #4ECDC4;">£{personal_b:,.2f}</p>
            <p style="font-size: 12px; color: #AAAAAA; margin: 0;">Personal Spending</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col3:
        st.markdown(f"""
        <div style="background-color: #262730; padding: 20px; border-radius: 10px; text-align: center; border: 2px solid #4A4A5E;">
            <h3 style="margin: 0; color: #FAFAFA;">🏠 Joint</h3>
            <p style="font-size: 32px; font-weight: bold; margin: 10px 0; color: #51CF66;">£{joint_total:,.2f}</p>
            <p style="font-size: 12px; color: #AAAAAA; margin: 0;">Shared Expenses</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    st.caption("Based on assigned spending, not whose card was used")
    
    # Responsibility breakdown - each person owes their personal + half of joint
    person_a_responsibility = personal_a + (joint_total / 2)
    person_b_responsibility = personal_b + (joint_total / 2)
    
    # Calculate total bill
    total_bill = grand_total
    
    # If splitting 50/50, each person should pay half the total
    fair_share = total_bill / 2
    
    # Settlement is the difference between what each person owes vs fair 50/50 split
    # If Person A owes more, they should pay Person B the difference to balance it
    settlement = person_a_responsibility - fair_share
    
    if abs(settlement) < 0.01:
        st.success("✅ You're all squared up! Spending is perfectly balanced.")
    elif settlement > 0:
        # Person A spent more, so Person B owes Person A
        st.warning(f"**Person B owes Person A: £{abs(settlement):,.2f}**")
    else:
        # Person B spent more, so Person A owes Person B
        st.warning(f"**Person A owes Person B: £{abs(settlement):,.2f}**")
    ...
    with st.expander("📝 Settlement Calculation Details"):
        ...
        **Person A's Responsibility:**
        ...
        - **Total Owes**: £{person_a_responsibility:,.2f}
        
        **Person B's Responsibility:**
        - Personal Expenses: £{personal_b:,.2f}
        - Joint share (50%): £{joint_total/2:,.2f}
        - **Total Owes**: £{person_b_responsibility:,.2f}
        
        **Settlement Logic:**
        - Total Bill: £{total_bill:,.2f}
        - Fair 50/50 split: £{fair_share:,.2f} each
        - Person A owes: £{person_a_responsibility:,.2f}
        - Person B owes: £{person_b_responsibility:,.2f}
    ...
    # Add detailed transaction view for debugging
    with st.expander("🔍 All Extracted Transactions (Debugging)"):
        st.caption("Review all transactions extracted from the PDF to verify accuracy")
        ...
        # Reorder columns
        display_cols = ['date', 'description', 'amount_display', 'cardholder', 'category']
        df_display = df[display_cols].copy()
        df_display.columns = ['Date', 'Description', 'Amount', 'Cardholder', 'Category']
        
        st.dataframe(df_display, use_container_width=True, hide_index=True, height=400)
        
        # Show verification totals
        st.write("**Verification Totals:**")
        col1, col2 = st.columns(2)
        with col1:
            st.write(f"- Sum of all amounts: £{grand_total:,.2f}")
            st.write(f"- Brigita's card total: £{paid_by_a:,.2f}")
            st.write(f"- Ravinder's card total: £{paid_by_b:,.2f}")
        with col2:
            st.write(f"- Person A category: £{personal_a:,.2f}")
            st.write(f"- Person B category: £{personal_b:,.2f}")
            st.write(f"- Joint category: £{joint_total:,.2f}")
        
        st.info("💡 **Tip:** If totals don't match your statement, check for missing transactions or duplicates in the table above.")
