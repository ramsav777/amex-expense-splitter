...
def extract_transactions(pdf_path: str) -> List[Dict[str, any]]:
    ...
    for idx, line in enumerate(all_lines):
        ...
        uk_pattern = r'(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)(\d{1,2})\s+(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)(\d{1,2})\s+(.+?)\s+([\d,]+\.\d{2})\s*(CR)?(?:\s|$)'
        ...
        if uk_match:
            ...
            # Robust CR detection: check if ' CR' is anywhere in the line
            line_upper = line.upper()
            is_credit = ' CR' in line_upper or line_upper.endswith('CR')
        ...
        us_pattern = r'(\d{1,2}/\d{1,2}(?:/\d{2,4})?)\s+(.+?)\s+\$?([\d,]+\.\d{2})\s*(CR)?(?:\s|$)'
